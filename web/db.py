...
conn = psycopg2.connect(host=os.environ['DATABASE_URL'], database='marilde',
...
def editaisDetalhes(id_ed):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT * FROM vEditais_com_detalhes WHERE id_edital = %s;", [id_ed])
        ed = cur.fetchone()
        print(ed)
        return ed
...
def checaPessoa(cpf):
    with conn.cursor() as cur:
        ...
        cur.execute(
            "SELECT id_pessoa, senha, nome FROM vCpfPassaporte WHERE cpfoupass = %s;", [cpf])
        dados = cur.fetchone()
        print(dados)
        return dados
...
def criaPessoa(nome, cpf, senha, estado, cidade, bairro, rua, numero, funcao):
    ...
    id_novo = -1
    with conn.cursor() as cur:
        cur.execute("INSERT INTO pessoa (nome, senha, uf, cidade, bairro, rua, numero) VALUES (%s, %s, %s, %s, %s, %s, %s);", [
            nome, senha, estado, cidade, bairro, rua, numero])
        cur.execute("SELECT id_pessoa FROM Pessoa WHERE nome = %s", [nome])
        id_novo = cur.fetchone()[0]

        cur.execute("INSERT INTO pessoabrasileira (cpf, id_pessoa) VALUES (%s, %s);", [
            cpf, id_novo])
        if funcao == "Discente":
            cur.execute("INSERT INTO PessoaGraduacao (nro_ufscar, id_pessoa) VALUES (%s, %s);", [
                id_novo, id_novo])
    conn.commit()
    return id_novo
